              ...
              <div v-for="bf in brandLogoFields" :key="bf.key" class="space-y-1">
                ...
                <div class="flex items-center gap-2">
                  <span class="text-[11px] text-zinc-500 flex-1 truncate">
                    {{ formData[bf.key] || bf.placeholder }}
                  </span>
                  ...
                  <button
                    ...
                    type="button"
                    @click="handleLogoDelete(bf.key)"
                  >
                    删除
                  </button>
                </div>
                <!-- 预览 -->
                <div v-if="formData[bf.key]" class="flex items-center gap-3 p-2 bg-zinc-950 rounded-lg border border-zinc-800/50">
                  <img
                    :src="formData[bf.key]"
                    :alt="bf.label"
                    class="w-12 h-12 object-contain rounded bg-zinc-900 p-1"
                    @error="$event.target.style.display='none'"
                  >
                  <span class="text-[10px] text-zinc-500 truncate flex-1">{{ formData[bf.key] }}</span>
                </div>
              </div>
            </div>
          </div>
...
const brandLogoFields = [
  { key: 'brandLogoPngUrl', label: '品牌 Logo (PNG)', placeholder: '替换 public/images/logo.png', type: 'brand-png', accept: 'image/png,image/jpeg,image/webp' },
  { key: 'brandLogo144Url', label: '品牌 Logo 144px', placeholder: '替换 public/images/logo-144.png', type: 'brand-144', accept: 'image/png' },
  { key: 'brandLogoSvgUrl', label: '品牌 Logo (SVG)', placeholder: '替换 public/images/logo.svg', type: 'brand-svg', accept: 'image/svg+xml' }
]
...
const loadConfig = async () => {
  try {
    ...
    formData.value = {
      ...
      brandLogoPngUrl: data.brandLogoPngUrl || '',
      brandLogo144Url: data.brandLogo144Url || '',
      brandLogoSvgUrl: data.brandLogoSvgUrl || '',
      ...
      allowPhoneRegistration: !!data.allowPhoneRegistration,
      smsEnabled: !!data.smsEnabled,
      smsProvider: data.smsProvider || 'aliyun',
      smsAliyunAccessKeyId: data.smsAliyunAccessKeyId || '',
      smsAliyunAccessKeySecret: data.smsAliyunAccessKeySecret || '',
      smsAliyunSignName: data.smsAliyunSignName || '',
      smsAliyunTemplateCode: data.smsAliyunTemplateCode || '',
      ...
      qqOAuthEnabled: !!data.qqOAuthEnabled,
      qqClientId: data.qqClientId || '',
      qqClientSecret: data.qqClientSecret || '',
